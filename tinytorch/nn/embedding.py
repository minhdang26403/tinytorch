...
class PositionalEncoding:
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if len(x.shape) != 3:
            raise ValueError(
                f"Expected 3D input (batch, seq, embed), got shape {x.shape}"
            )
        ...
        if embed_dim != self.embed_dim:
            raise ValueError(
                f"Embedding dimension mismatch: expected {self.embed_dim}, got "
                f"{embed_dim}"
            )
    ...
    def __repr__(self):
        return (
            f"PositionalEncoding(max_seq_len={self.max_seq_len}, "
...
class EmbeddingLayer:
    ...
    def __init__(
        ...
        if pos_encoding == "learned":
            ...
            raise ValueError(
                f"Unknown pos_encoding: {pos_encoding}. Use 'learned', 'sinusoidal', or None"
            )
